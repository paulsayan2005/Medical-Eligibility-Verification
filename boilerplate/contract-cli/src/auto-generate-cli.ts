...
class CLIAutoGenerator {
  ...
  private generateAPIWrapperContent(analysis: any): string {
    ...
    ${Object.entries(analysis.ledgerState)
      .map(([name, type]) => `  ${name}: ${this.mapToTypeScriptType(type as string)};`)
      .join('\n')}
    }
    `;
  }
  ...
  private generateFunctionWrapper(func: any): string {
    const paramList = func.parameters.map((p: any) => `${p.name}: ${this.mapToTypeScriptType(p.type)}`).join(', ');
