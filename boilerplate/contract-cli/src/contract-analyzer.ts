...
export class ContractAnalyzer {
  ...
  async analyzeContract(): Promise<ContractAnalysis> {
    try {
      ...
      if (fs.existsSync(witnessesPath)) {
        ...
        const witnessLineRegex =
          /(\w+):\s*\(\{[^}]*\}\s*:\s*WitnessContext<([\w.]+|typeof [\w.]+),\s*([^>]+)>\)\s*:\s*[^=]+=>[\s\n]*\[((?:.|\n)*?)\][,\n]?/gs;
        let match;
   ...
   */
  private parseFunctionSignatures(functionsBlock: string, functions: ContractFunction[], _isReadOnly: boolean): void {
    // Parse each function signature - updated regex to handle the actual format
    const functionRegex =
      /(\w+)\(context:\s*__compactRuntime\.CircuitContext<T>(?:,\s*([^)]+))?\):\s*__compactRuntime\.CircuitResults<T,\s*([^>]+)>/g;
    let match;
  ...
  isReadOnlyFunction(functionName: string): boolean {
    ...
    const hasReturnValue =
      func.returnType && func.returnType !== '[]' && func.returnType !== 'void' && func.returnType.trim() !== '';
